...
for letter in string.ascii_lowercase[:17]: 
    with open('./jobfiles/coref_' + letter + '_dating.job', 'w') as outfile:
        outfile.write('#!/bin/bash\n')
        outfile.write('# Job name:\n')
        outfile.write('#SBATCH --job-name=coref_' + letter + '_dating\n')
        outfile.write('# Partition:\n')
        outfile.write('#SBATCH --partition=savio\n')
        outfile.write('#\n')
        outfile.write('# Wall clock limit:\n')
        ...
        outfile.write('time awk -F "," \'{print $1}\' ../../logs/data_splits/xa' + letter + ' | parallel --joblog ../../logs/coref_tasklogs/task_' + letter + '_dating.log --resume \'python ../coref_reddit_dating.py {}\' &\n')
